...
\begin{equation}
  ...
  =\int_{\Omega} d\Omega\int_0^{r_{\mathrm{max}}} dr' \frac{\mathcal{E}(\mathbf{r}'+\mathbf{x}) }{4\pi } e^{-\kappa r},
\end{equation}
where the emissivity $\mathcal{E}$ is an energy per unit time, per unit volume, $\tau=\kappa (\mathbf{x}'-\mathbf{x})$ is the optical depth along the line of sight. In the last step, we introduced $\mathbf{r'}=\mathbf{x}'-\mathbf{x}$, $d\Omega=\sin\theta d\theta d\phi$  and $\kappa$ the absorption coefficient. The differential heating rate is defined as $d\dot{Q}(\mathbf{x})\equiv dJ(\mathbf{x})/D_{\mathrm{pp}}$ such that the volume-integrated heating rate $\dot{Q}$ and its average (in units of erg cm$^{-3}$ s$^{-1}$) are given by 
\begin{equation}
  \label{eq:heating_rate0}
  \dot{Q}(\mathbf{x})=
  \frac{1}{4\pi}   \int_{\Omega}d\Omega\int_0^{r_{\mathrm{max}}} dr' \mathcal{E}(\mathbf{r}'+\mathbf{x}) \frac{1}{D_{\mathrm{pp}}} e^{-\kappa r},\quad\mbox{and}\quad
  \bar{\dot{Q}}= \frac{1}{4\pi}   \int_{\Omega}d\Omega\int_0^{r_{\mathrm{max}}} dr' \bar{\mathcal{E}} \frac{1}{D_{\mathrm{pp}}} e^{-\kappa r},
\end{equation}
...
\end{eqnarray}
\ALc{Performing a convolution of the density fluctuations with the kernel $C r^{-2} e^{-r/D_{\mathrm{pp}}}$ (with C an arbitrary constant), and using the Fourier convolution theorem, Eq. \ref{eq:heat_fluc_newt0} rewrites}
...
\end{equation}
where $J_E$ is the specific intensity at the origin (in erg s$^{-1}$ cm$^{-2}$ erg$^{-1}$), $E_{\mathrm{min}}$ to $E_{\mathrm{max}}$ is the range over which pair production is possible. Its evolution is dictated by the equation of radiative transfer in an expanding universe
\begin{equation}\label{eq:radiative transfer}
  \frac{\partial J_E}{\partial t} - E H \frac{\partial J_E}{\partial E} + 3 H J_E + c\kappa J_E  = \frac {c}{4\pi} \epsilon_E,
\end{equation}
where $H = \dot{a}/a$ is the Hubble parameter, $a$ is the scale factor, $\kappa$ is the , and $\epsilon_E$ is the specific proper emissivity.  Quantities with a \textasciicircum are measured in the comoving frame.
...
\end{equation}
where $\hat r$ is the comoving distance, $\hat{\mathcal{E}}_E = \epsilon_E/(1+z')^3$ is the comoving specific emissivity, $E' = E (1+z')/(1+z)$ and $\tau$ is the approximate optical depth given by
\begin{equation}
\label{eq:tau}
\tau(E,z,z')\equiv \tau=\int_z^{z'}dz''\frac{1}{\hat D_{\mathrm{pp}}(E,z'')}\frac{d\hat r}{dz''}.%=\int_z^{z'}dz''\frac{c}{H(z'')}\frac{1}{D_{\mathrm{pp}}},
\end{equation}
...
\bar{\dot{Q}}=\frac{(1+z)^3}{4\pi}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(z)}\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz' \frac{d\hat r}{dz'}\bar{\hat{\mathcal{E}}}(z') e^{-\tau(z,z')}.
\end{equation}

\begin{eqnarray}
\label{eq:fluc_exp0}
\delta_H(z)&=&\frac{\dot{Q}(z)-\bar{\dot{Q}}}{\bar{\dot{Q}}}=\frac{(1+z)^3c}{4\pi\bar{\dot{Q}}} \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}}\frac{dE}{\hat D_{\mathrm{pp}}(z)}\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz'\frac{ ( \mathcal{\hat E}(z')-\bar{\hat{\mathcal{E}}}(z')) e^{-\tau(z,z')}}{H(z')} \\ \nonumber
&=&\frac{(1+z)^3 c}{4\pi\bar{\dot{Q}}} \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(z)}\int_{\Omega}d\Omega\int_0^{z_{\mathrm{max}}} dz' \frac{\bar{\hat{\mathcal{E}}}(z')\delta_E(z') e^{-\tau(z,z')}}{H(z')},
\end{eqnarray}
where we have removed the energy dependencies for more clarity.
...
At this stage we assume the TeV fluctuations, which mathc the baryonic density fluctuations exactlty match the dark matter fluctuations $\delta_E=\delta$. We explain in the next section that this is not exactly true and will take into account various corrections. The initial density fluctuations represent a Gaussian random field, which exact properties depend on the earliest stages of the universe prior to recombination \citep{1986ApJ...304...15B,Peebles}. They grow linearly between $z'$ and $z$ following  \citep{ 1977MNRAS.179..351H}
\begin{equation}
  \label{eq:growth}
...
\delta_H(z,\hat r)=\frac{(1+z)^3  c}{4\pi\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(z)} \int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}}dz' \frac{D(z')}{D(z)} \frac{\bar{\hat{\mathcal{E}}}(z')\delta(z',\hat r) e^{-\tau(z,z')}}{H(z')}.
\end{equation}
...
\tilde{\delta}_H(z,\hat k)=\tilde{\delta}(z,\hat k) \frac{(1+z)^3c}{4\pi\bar{\dot{Q}}} \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(z)}\int_z^{z_{\mathrm{max}}} dz' \frac{D(z')}{D(z)}\frac{\sin(\hat k\hat r'(z'))}{\hat k \hat \hat r'(z')} \frac{\bar{\hat{\mathcal{E}}}(z') e^{-\tau(z,z')}} {H(z')}.
\end{equation}
\begin{figure}[h]
\centering
...
\end{equation}
with $\mu$ the cosine of the angle between the wavenumber and the line of sight.  $f\equiv f(\Omega)\equiv d\log\delta/d\log a$, with $\Omega$ the cosmological density parameter.
Keeping only first order correction for density fluctuations, Eq.~\eqref{eq:int_exp_heat} yields
\begin{equation}
\label{eq:mean_heat0}
\dot{Q}(z)=\frac{(1+z)^3c}{4\pi }\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}}\frac{dE}{\hat D_{\mathrm{pp}}(z)}\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz'\frac{\mathcal{\bar{\hat E}}(z')e^{-\tau(z,z')}}{H(z')}\left[1+\left(\frac{D(z')}{D(z)}b(z)\delta(z,\hat r) -\frac{d\delta_{v_r}}{d\hat r}f\right)\right].
\end{equation}
...
\begin{equation}
\label{eq:def_X}
\frac{dX}{dz'}=\frac{(1+z)^3c}{4\pi \hat D_{\mathrm{pp}}(z)}\frac{\mathcal{\bar{\hat E}}(z')e^{-\tau(z,z')}}{H(z')},
\end{equation}
...
\begin{eqnarray}
\label{eq:heat_fluc}
\tilde{W}_H(z,\hat k)&=&\frac{(1+z)^3c }{\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(z)}\int_z^{z_{\mathrm{max}}} dz' \frac{\mathcal{\bar{\hat E}}(z')e^{-\tau(z,z')}}{H(z')}\frac{D(z')}{D(z)}\left[\left(b(z)+\frac{f}{3}\right)j_0(\hat k \hat r)-\frac{2}{3}fj_2(\hat k \hat r)\right]\\
&=& \frac{\displaystyle \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{{\hat D_{\mathrm{pp}}}(z)}\int_z^{z_{\mathrm{max}}} dz'   \frac{\mathcal{\bar{\hat E}}(z')e^{-\tau(z,z')}}{H(z')}\frac{D(z')}{D(z)}\left[\left(b(z)+\frac{f}{3}\right)j_0(\hat k \hat r)-\frac{2f}{3}j_2(\hat k \hat r)\right]}{\displaystyle \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{{\hat D_{\mathrm{pp}}}(z)}\int_z^{z_{\mathrm{max}}} dz'  \mathcal{\bar{\hat E}}(z') \frac{e^{-\tau(z,z')}}{H(z')}}.
\end{eqnarray}
