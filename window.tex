...
\int_z^{\infty} dz'   \frac{\mathcal{\bar{\hat E}}_{E'}(z') e^{-\tau}}{(1+z')\,H(z')}.
\end{eqnarray}
Here, $\hat{r}$ is the comoving distance between the source and heated region, $D$ denotes the linear growth factor defined in Eq.~\eqref{eq:growth_1}, $f\equiv d\log \delta/d \log a$, $E$ is the energy of the received TeV photon, $E'$ its initial energy and $\mathcal{E}$ the (comoving) blazar luminosity density, $b$ is the Eulerian bias and $j_0$ and $j_2$ are spherical Bessel functions. \Cc{Is $\mathcal{\bar{\hat E}}_{E'} = \mathcal{\bar{\hat E}} (E')/E'$?}
...
%Assuming blazars follow a similar evolution to quasars, we use $z_\mathrm{max}=5$ \citep{2007ApJ...654..731H}.
...
The heating rate in an expanding universe at point $\mathbf{\hat{x}}$ is given by
\begin{equation}
  \dot{Q}(\mathbf{\hat{x}})= \int_{E_{\rm min}}^{E_{\rm max}} dE \frac{J_E(\mathbf{\hat{x}})}{D_{\mathrm{pp}}(E, z)},
\end{equation}
where $z$ is the redshift at $\mathbf{\hat{x}}$ and the interval
$E_{\mathrm{min}}$ to $E_{\mathrm{max}}$ is the energy range over which pair
...
\begin{equation}\label{eq:expanding J}
  J_E(E, \mathbf{\hat{x}}) = \frac {(1 + z)^3} {4\pi} \int_{\Omega(\btheta)} d\Omega \int_z^{\infty} \frac{d z'}{1+z'}\,\frac{d\hat{r}}{dz'}\, {\hat{\mathcal{E}}_{E'}(\mathbf{\hat{r}}+\mathbf{\hat{x}})}e^{-\tau(E,z,z')},
\end{equation}
where the vector $\mathbf{\hat{r}} = [\hat{r}(z'), \mathbf{\hat{r}}_\perp(\btheta)]$ contains the radial comoving distance $\hat r$ and comoving perpendicular distance $\mathbf{\hat{r}}_\perp$, $z$ is the absorption redshift, $\hat{\mathcal{E}}_E = \epsilon_E/(1+z')^3$ is the comoving specific emissivity defined in Eq.~\eqref{eq:mean_heat}, $E' = E (1+z')/(1+z)$ and $\tau$ is the approximate optical depth given by
\begin{equation}
\label{eq:tau}
...
\begin{equation}
  ...
  \dot{Q}[\mathbf{\hat{x}}(z)]=\frac{(1+z)^3}{4\pi }\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{D_{\mathrm{pp}}(E,z)}\int_{\Omega}d\Omega\int_z^{\infty} \frac{d z'}{1+z'}\,\frac{d\hat r}{dz'}\mathcal{\hat E}_{E'}(z',\btheta) e^{-\tau(E,z,z')}.
\end{equation}
For clarity, we omit here and in the following the explicit dependencies of the specific emissivity $\hat{\mathcal{E}}_{E'}[\mathbf{\hat{r}}(z',\btheta)+\mathbf{\hat{x}}]$ and only show the implicite ones that are being integrated over. Similarly to Eq.~\eqref{eq:heating_rate0}, the mean heating rate is given by
\begin{equation}
...
\begin{eqnarray}
\label{eq:fluc_exp0}
\delta_H[\mathbf{\hat{x}}(z)]&=&\frac{\dot{Q}[\mathbf{\hat{x}}(z)]-\bar{\dot{Q}}(z)}{\bar{\dot{Q}}(z)}=\frac{(1+z)^3c}{4\pi\bar{\dot{Q}}} \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}}\frac{dE}{D_{\mathrm{pp}}(E,z)}\int_{\Omega}d\Omega\int_z^{\infty} d z'\,\frac{ \left[ \mathcal{\hat E}_{E'}(z',\btheta)-\bar{\hat{\mathcal{E}}}_{E'}(z')\right] e^{-\tau(E,z,z')}}{(1+z')\,H(z')} \nonumber\\ 
&=&\frac{(1+z)^3 c}{4\pi\bar{\dot{Q}}} \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{D_{\mathrm{pp}}(E,z)}\int_{\Omega}d\Omega\int_z^{\infty} d z'\, \frac{\bar{\hat{\mathcal{E}}}_{E'}(z')\delta_E(z',\btheta) e^{-\tau(E,z,z')}}{(1+z')\,H(z')},
\end{eqnarray}
...
\begin{equation}
  \label{eq:growth}
\delta(z,\btheta)=\delta(z',\btheta)D(z)/D(z'),
\end{equation}
...
\begin{equation}
\label{eq:FT_delta}
\delta_E[\mathbf{\hat{r}}(z',\btheta)]=\delta[\mathbf{\hat{r}}(z',\btheta)]
=\delta[\mathbf{\hat{r}}(z,\btheta)]\frac{D(z')}{D(z)}
...
\delta_H[\mathbf{\hat{x}}(z)]=\frac{(1+z)^3  c}{4\pi\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{D_{\mathrm{pp}}(E,z)} \int_{\Omega}d\Omega\int_z^{\infty}dz' \frac{D(z')}{D(z)} \frac{\bar{\hat{\mathcal{E}}}_{E'}(z')\delta(z,\btheta) e^{-\tau(E,z,z')}}{(1+z')\,H(z')}.
\end{equation}
...
\tilde{\delta}_H(z,\hat k)=\tilde{\delta}(z,\hat k) \frac{(1+z)^3c}{\bar{\dot{Q}}} \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{D_{\mathrm{pp}}(E,z)}\int_z^{\infty} dz' \frac{D(z')}{D(z)}\frac{\sin\left(\hat k\hat r(z')\right)}{\hat k \hat r(z')} \frac{\bar{\hat{\mathcal{E}}}_{E'}(z') e^{-\tau(E,z,z')}} {(1+z')\,H(z')}.
\end{equation}
\begin{figure}[h]
\centering
...
dz'=d\hat r\frac{H(z')}{c}\left(1-\frac{d\delta_{v_r}(z')}{d\hat r}\right),
\end{equation}
...
\begin{equation}
\label{eq:mean_heat0}
\dot{Q}[\mathbf{\hat{x}}(z)]=\frac{(1+z)^3c}{4\pi }\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}}\frac{dE}{D_{\mathrm{pp}}(E,z)}\int_{\Omega(\btheta)}d\Omega\int_z^{\infty} dz'\frac{\mathcal{\bar{\hat E}}_{E'}(z')e^{-\tau(E,z,z')}}{(1+z')\,H(z')}\left[1+\frac{D(z')}{D(z)}b(z')\delta[\mathbf{\hat{r}}(z',\btheta)+\mathbf{\hat{x}}] -\frac{d\delta_{v_r}}{d\hat r}f\right].
\end{equation}
Subtracting the mean heating rate (Eq.~\eqref{eq:mean_exp_heat}) then yields the fluctuations
\begin{equation}
\label{eq:heat_fluc0}
\delta_H[\mathbf{\hat{x}}(z)]=\frac{1}{\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} dE\int_{\Omega}d\Omega\int_z^{\infty}dz'X(E,z,z')\left[\frac{D(z')}{D(z)}b(z')\delta[\mathbf{\hat{r}}(z',\btheta)+\mathbf{\hat{x}}] -\frac{d\delta_{v_r}}{d\hat r}f\right],
\end{equation}
where we introduced
\begin{equation}
\label{eq:def_X}
X(E,z,z')=\frac{(1+z)^3c}{4\pi D_{\mathrm{pp}}(E,z)}\frac{\mathcal{\bar{\hat E}}_{E'}(z')e^{-\tau(E,z,z')}}{(1+z')\,H(z')}
\end{equation}
for convenience. Transforming to Fourier space yields
\begin{eqnarray}
\label{eq:heat_fluc0}
\tilde{\delta}_H(z,\mathbf{\hat k})&=&\frac{1}{\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} dE\int_{\Omega}
...
\left[\frac{D(z')}{D(z)}b(z')\tilde\delta(z,\mathbf {\hat k'}) -\frac{d\delta_{v_r}}{d\hat r}f\right].
\end{eqnarray}
Assuming statistical isotropy and homogeneity allows us to simplify this expression, and we arrive at
\begin{eqnarray}
\tilde{\delta}_H(z,\hat k)&=&\frac{2\pi\tilde{\delta}(z,\hat k)}{\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} dE \int_{\Omega}d\Omega\int_z^{\infty} dz'X(E,z,z')\frac{D(z')}{D(z)}\int_{-1}^{1}d\mu\left[e^{-i\hat k \hat r\mu} (b +f \mu^2)\right]\\ \nonumber
&=&\frac{4\pi\tilde{\delta}(z,\hat k)}{\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} dE\int_z^{\infty}dz'X(E,z,z')\frac{D(z')}{D(z)}\left[\left(b(z')+\frac{f}{3}\right)j_0(\hat k \hat r)-\frac{2}{3}f j_2(\hat k \hat r)\right],
\end{eqnarray}
