...
\subsection {Determining the window function}\label{window}
...
To account for the Hubble expansion, we express the (comoving) integration through a redshift integration and account for the Jacobian, $dr=c dz /H(z)$, where  $H(z)$ is the Hubble parameter.  The energy-integrated,  volume-integrated heating rate is then given by 
...
E'=E\frac{1+z'}{1+z}.
\end{equation}
Similarly, $\tau(E,z,z')$ is the optical depth of a TeV photon observed at redshift $z$ with energy $E$, which was emitted at redshift $z'$ with energy $E'$.
\begin{equation}
\label{eq:tau}
