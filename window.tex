...
$J_E$ is the specific intensity per unit energy (in erg s$^{-1}$ cm$^{-2}$ erg$^{-1}$). \ALc{Assuming a flat geometry on spatial hypersurfaces of the Friedmann-Robertson-Walker metric, $J_E$ is} given by
...
\Cc{{\em [CP, PC: major parts rewritten to clarify the nature of redshift space distortions]}}
...
\begin{equation}
\hat n^{(\hat s)}(\mathbf{\hat s}) d^3\mathbf{\hat s} = \hat n(\calR) d^3\calR,
\qquad\mbox{or}\qquad
\left[1+\delta^{(\hat s)}(\mathbf{\hat s})\right] d^3\mathbf{\hat s} = 
\left[1+\delta(\calR)\right] d^3\calR,
...
\begin{equation}
1+\delta^{(\hat s)}(\mathbf{\hat s}) = 
\left[1+\delta(\calR)\right] \frac{\magcalR^2}{\left(\magcalR +v_{\mathcal{R}}/H(z)\right)^2}
...
\begin{equation}
  \label{eq:real_vs_z}
  \delta^{(\hat s)}\left[\mathbf{\hat s}(\calR) \right]=
  \delta(\calR)-\frac{1}{H(z)}\left(\frac{\partial}{\partial \magcalR} + 
    \frac{2}{\magcalR}\right)\hat{v}_{\mathcal{R}}\approx
  \delta(\calR)-\frac{1}{H(z)}\frac{\partial \hat{v}_{\mathcal{R}}}{\partial \magcalR},
\end{equation}
...
\begin{equation}
  \label{eq:Zeldovich}
  \delta^{(\hat s)}(\calR) = -\frac{f}{b H(z)}\frac{\partial \hat{\mathbf{v}}}{\partial \calR},
\end{equation}
...
\begin{equation}
  \label{eq:Zeldovich2}
  v_{\mathcal{R}} = - \frac{f}{b} \frac{\partial}{\partial \magcalR} \boldsymbol{\nabla}_{\calR}^{-2}\delta(\calR),
\end{equation}
where $\boldsymbol{\nabla}_{\calR}^{-2}$ represents the inverse Laplacian. It follows that
\begin{equation}
  \label{eq:Zeldovich3}
  \delta^{(\hat s)}[\calR(\bm\hat{\mathbf{r}})] = 
  \left[1+\frac{f}{b}\frac{\partial^2}{\partial \magcalR^2}
    \boldsymbol{\nabla}_{\calR}^{-2}
\right]\delta[\calR(\bm\hat{\mathbf{r}})] =
...
\begin{equation}
  \label{eq:real_vs_z2}
  \tilde{\delta}^{(\hat s)}[\mathbf{\hat{k}}_{\hat s}(\mathbf{\hat{k}})]=
  \left(1+\frac{f}{b}\mu^2\right) \tilde{\delta}(\mathbf{\hat{k}}),
\end{equation}
...
\left[1+b(z')\left(\delta(\calR, z') -\frac{1}{H(z')}
\frac{\partial v_{\mathcal{R}}}{\partial \magcalR}\right)\right].
\end{equation}
...
\int_{\Omega}d\Omega\int_z^{\infty}dz'X(E,z,z')
b(z')\left[\delta(\calR, z') -\frac{1}{H(z')}
\frac{\partial v_{\mathcal{R}}}{\partial \magcalR}\right],
\end{equation}
where we introduced
\begin{equation}
\label{eq:def_X}
