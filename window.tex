...
\end{equation}
An alert reader will recognize that the true optical depth must include the effect of a changing \Pc{$E'' = E(1+z'')/(1+z)$ as we integrate along $z''$ from $z$ to $z'$}.  \Pc{However, h}ere we assume the typical mean free paths for high energy photons are much smaller than the Hubble length so that \Pc{$E'' \approx E$} over the redshift interval is small compared to $z$  where integrand is nontrivial.  
