...
The latter is expected due to He\,\textsc{ii} reionization \citep[e.g.][]{2009ApJ...694..842M,2013MNRAS.435.3169C,2014arXiv1410.1531P}. However, recent measurements found that underdense regions may be warmer than predicted \citep{2009MNRAS.399L..39V,2008MNRAS.386.1131B}, as well as \Cc{to exhibit} unexpectedly high temperatures at $z<2$ \citep{2014MNRAS.441.1916B}. Although an inverted temperature-density relation in underdense regions has not been firmly established yet \citep{2014MNRAS.438.2499B}, these observations suggest that the thermal history of the IGM may be more complex than initially assumed.
...
To model cosmic distances, Eq.~\eqref{eq:heating_rate} is integrated in redshift space and we take into account the resulting energy loss for the TeV photons as well as first order corrections due to \Cc{a clustered blazar distribution and} proper motions of the sources within the Hubble flow \citep{1987MNRAS.227....1K}. We integrate over the energy distribution of the TeV-emission. Following the detailed derivation in the Appendix, we obtain the window function $\tilde{W}_H(\hat{k},z)$ as a function of comoving wavelength $\hat{k}$,
\begin{eqnarray}
\label{eq:window}
...
\Cc{$\hat{\mathcal{E}}_E$} the (comoving) blazar spectral luminosity density, $b$
...
The cosmological model is based on the \textit{WMAP} 7-year data \citep{2011ApJS..192...18K}: $\Omega_M=0.272$, $\Omega_{\Lambda}=0.728$, $\Omega_{B}= 0.0465$, $h=0.704$ and $\sigma_8=0.809$. The initial conditions were evolved from $z=100$ until $z=1$ in boxes with comoving side length of 100 $h^{-1}$ Mpc and periodic boundary conditions. We use $N= 2\times 512^3$ particles, which gives a mass of $m_\mathrm{gas}=3.8\times10^{6} h^{-1} M_{\odot}$ and $m_\mathrm{DM}=1.8\times 10^{7} h^{-1} M_{\odot}$ for baryonic and dark matter particles, respectively. We used a gravitational softening length of 7.8 $h^{-1}$ kpc. We checked that the resolution has limited impact on the results of our simulations, performing test simulations with a comoving side length of 50 $h^{-1}$ Mpc. \Cc{We checked that the resolution has a barely discernible impact on the results of our simulations by performing test simulations with a comoving side length of 50 $h^{-1}$ Mpc at different resolutions, up to $2\times 512^3$. The spread of the temperature in the low density regions increases with increasing resolution. However, this effect is approaching saturation at a resolution of $N=2\times 256^3$. We find less than $4\%$ difference in the median temperature and less than $10\%$ difference in its root mean square in low density regions between $N=2\times 256^3$ and $2\times 512^3$ simulation indicating that our chosen resolution is sufficient to accurately capture the temperature-density distribution. \sout{Increasing the resolution significantly broadens the temperature distribution of the gas for $N < 2\times 256^3$. However, there is a less than $4\%$ difference in the median temperature and less than $10\%$ difference in its root mean square in low density regions between $N=2\times 256^3$ and $2\times 512^3$, indicating the resolution we use properly captures the temperature-density distribution.}}
...
\tilde{\delta}_H(k) = \tilde{\delta}_E(k) \tilde{W}(k),
 \end{equation}
where the \ALc{tilde} denotes quantities in Fourier space and $\tilde{W}(k)$ is the Fourier transform of $C r^{-2} e^{-r/D_{\mathrm{pp}}}$. This yields
...
\includegraphics[width = .45\textwidth ]{newtonian_window}
\caption{Window function for a \ALc{monochromatic source in a } non-expanding universe \ALc{(Eq.~\eqref{eq:heat_fluc_newt3})} . The dashed red line
...
\caption{Window function for blazar heating in an expanding universe \Cc{without account for clustering bias} \ALc{(Eq.~\eqref{eq:heat_fluc_exp1})} for $z=1$ (solid line), 2 (dashed line) and 4 (dotted line).}
...
\Pc{As discussed in the main text, the rapidly decreasing blazer luminosity density with increasing redshift for $z\gtrsim 2$, which move the break toward larger wave vector dominates the comoving mean free path, which move the break toward smaller wave vectors.  This is due to the fact that the integration over the energy of the photons, which includes lower energy photons with larger mean free paths, allows the former effect to dominate and moves the break toward smaller wave vectors with increasing redshift.}
...
%{[{ I removed rewrote the section about the redshift space distortions. I only kept the final equations rather than the complete derivation with Jacobian. I'm not fully sure about unifying the notations with the rest (for hats and stuff in paranthesis. We actually use z as a space coordinate in this part but as an indication of time in all the rest of the appendix). I corrected Eq. C4 and 5}]}\sout{ This yields the following redshift correction}}
% \begin{equation}
% \label{eq:vel_perturb}
% dz'=d\hat r\frac{H(z')}{c}\left(1+\frac{dv_r(z')}{d\hat r}\right),
% \end{equation}
%where $dv_r$ are the velocity perturbations along the line of sight. The Fourier transform of $v_r$ gives \citep{1987MNRAS.227....1K}.
% \begin{equation}
% \label{eq:kaiser2}
% \mathcal{F}\left(\frac{dv_r}{d\hat r}\right)=\mu^2 f \tilde{\delta}(\mathbf{\hat k}),
% \end{equation}
% }
\ALc{Assuming the scale of the fluctuations is small compared to the distance to the sources,}   we have  
\begin{equation}
  \label{eq:real_vs_z}
  \delta_s(\mathbf{\hat{x}})=\delta(\mathbf{\hat{x}})-\frac{\partial }{\partial \hat{r}}\left(\frac{v_r}{H_0}\right).
\end{equation}
\ALc{where $v_r$ is the radial component of the peculiar velocity \citep{MFW}.} In Fourier space, this gives
\begin{equation}
  \label{eq:real_vs_z}
