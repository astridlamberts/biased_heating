...
\end{equation}
where \Pc{\sout{$z$ is the redshift at $\mathbf{\hat{x}}$ and}} the interval
...
\end{equation}
where the vector \Pc{\sout{$\mathbf{\hat{r}} = [\hat{r}(z'), \mathbf{\hat{r}}_\perp(\btheta)]$} $\mathbf{\hat{r}}(\hat{r}(z,z'), \theta, \phi)$ \sout{contains the radial comoving distance $\hat r$ and comoving perpendicular distance $\mathbf{\hat{r}}_\perp$} is a function of the radial comoving distance $\hat{r}$ and position angles, $\theta$ and $\phi$}, $z$ is the absorption redshift, $\hat{\mathcal{E}}_E = \epsilon_E/(1+z')^3$ is the comoving specific emissivity defined in Eq.~\eqref{eq:mean_heat}, $E' = E (1+z')/(1+z)$ and $\tau$ is the approximate optical depth given by
\begin{equation}
\label{eq:tau}
\tau(E,z,z')\equiv \tau=\int_z^{z'}dz''\frac{1}{\hat D_{\mathrm{pp}}(E,z'')}\frac{d\hat r}{dz''}.
\end{equation}
An alert reader will recognize that the true optical depth must include the effect of a changing \Pc{\sout{$\hat D_{\rm pp}$} $E'$} as we integrate from $z$ to $z'$.  Here we \Pc{ assume \sout{make the assumption that $\tau(E',z,z') \propto \hat r$ to derive equation (\ref{eq:expanding J}). This assumption holds  because}} the typical mean free paths for high energy photons are much smaller than the Hubble length\Pc{\sout{.  Hence} so that } $E' \approx E$ \Pc{\sout{as} over} the redshift interval \Pc{ is small compared to $z$ \sout{over which} where integrand is nontrivial \sout {is small compared to $z$}}.  
...
\end{equation}
\Pc{where $\mathbf{\hat{\mathcal{R}}} = \mathbf{\hat{x}} + \mathbf{\hat{r}}$.  \sout{For clarity, we omit here and in the following the explicit dependencies of the specific emissivity $\hat{\mathcal{E}}_{E'}[\mathbf{\hat{r}}(z',\btheta)+\mathbf{\hat{x}}]$ and only show the implicite ones that are being integrated over.}} Similarly to Eq.~\eqref{eq:heating_rate0}, the mean heating rate is given by
\begin{equation}
...
\begin{equation}
\label{eq:mean_heat0}
\dot{Q}(\mathbf{\hat{x}},z)=\frac{(1+z)^3c}{4\pi }\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}}\frac{dE}{D_{\mathrm{pp}}(E,z)}\int_{\Omega}d\Omega\int_z^{\infty} dz'\frac{\mathcal{\bar{\hat E}}_{E'}(z')e^{-\tau(E,z,z')}}{(1+z')\,H(z')}\left[1+\frac{D(z')}{D(z)}b(z')\delta(\mathbf{\hat{\mathcal{R}}}, z') -\frac{d\delta_{v_r}}{d\hat r}f\right].
\end{equation}
Subtracting the mean heating rate (Eq.~\eqref{eq:mean_exp_heat}) then yields the fluctuations
\begin{equation}
\label{eq:heat_fluc0}
\delta_H(\mathbf{\hat{x}}, z)=\frac{1}{4\pi\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} dE\int_{\Omega}d\Omega\int_z^{\infty}dz'X(E,z,z')\left[\frac{D(z')}{D(z)}b(z')\delta(\mathbf{\hat{\mathcal{R}}}, z') -\frac{d\delta_{v_r}}{d\hat r}f\right],
\end{equation}
where we introduced
\begin{equation}
\label{eq:def_X}
