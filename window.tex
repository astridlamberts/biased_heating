...
\begin{equation}
\label{eq:define_X}
X(E,z',z)=\frac{\mathcal{E}(E',z') e^{-\tau(E',z,z')}}{H(z') D_{\mathrm{pp}}(E',z')},
\end{equation}
with $r$ the comoving distance between the source and heated region, $\bar{X}$ its spectral average, $D$ denotes the linear growth factor defined in Eq.~\eqref{eq:growth_1}, $E$ is the energy of the received TeV photon, $E'$ its initial energy and $\mathcal{E}$ the (comoving) blazar luminosity density, $r$ the comoving distance between the source and heated region, $b$ is the bias and $j_0$ and $j_2$ are spherical Bessel functions.

Assuming the TeV blazar distribution follows the redshift evolution of quasars, \citet{2012ApJ...752...23C} determined the comoving blazar luminosity density in the TeV band based on a fit to \citet{2007ApJ...654..731H}
\begin{eqnarray}
\label{eq:mean_heat}
\hat{\mathcal{E}}(E',z)&=&\mathcal{E}(E',0)\hat\Phi_{B}(z)\\ \nonumber
&\simeq& \mathcal{E}(E',0)\zeta\hat\Phi_{Q}(z),
\end{eqnarray}
with $\zeta=2.1\times 10^{-3}$ and \Cc{parametrized comoving quasar luminosity density is given by}
\begin{equation}
\label{eq:phi_quasar}
...
\begin{equation}
\label{eq:blaz_lum}
\mathcal{E}(E',0)=\mathcal{E}_0\,F_{E'},
\end{equation}
with $\mathcal{E}_0=(1.7-4.8)\times 10^{-36}$erg s$^{-1}$ cm$^{-3}$ the current blazar luminosity density \Cc{and $F_E$ denotes intrinsic TeV blazar spectrum \citep{2014ApJ...790..137B}
\begin{equation}
\label{eq:intrinsic_spectrum}
F_E = \frac{f_0 E}{(E/E_b)^{\Gamma_l} + (E/E_b)^{\Gamma_h}},
\end{equation}
where $f_0$ is set by requiring $\int_{E_{\rm min}}^{E_{\rm max}} dE F_E = 1$ (all energies are measured in units of TeV) and $\Gamma_h=3$. We averaged $F_E$ over the Gaussian distribution in $\Gamma_l$ (low-energy photon spectral index) from \citet{2014ApJ...790..137B}. Fitting the resulting distribution by the broken power law flux model, allowing both $\Gamma_l$ and $E_b$ to vary, we minimized the chi-squared for $E F_E$ to emphasize the regions that dominate the energy flux. The resulting values are  $\Gamma_l=1.80$ and $E_b=1.057$, with an $L_1$ norm of 0.0048.Adopting typical parameters of nearby TeV blazar spectra yields $E_{\mathrm{min}}=100$ GeV, and $E_{\mathrm{max}}=10$ TeV. The spectral shape of the EBL selects $E_{\mathrm{min}}=100$ GeV and $E_{\mathrm{max}}=10$ TeV and the limiting $\gamma$-ray energies for pair production.}
...
%\end{equation}
%with $\mathcal{E}_0=(1.7-4.8)\times 10^{-36}$erg s$^{-1}$ cm$^{-3}$ the current blazar luminosity density. Using 28 TeV blazars, \citet{2012ApJ...752...23C} find that $\alpha\simeq 3$ and $E_0\simeq $ 1 TeV. Adopting typical parameters of nearby TeV blazar spectra yields $E_{\mathrm{min}}=100$ GeV, and $E_{\mathrm{max}}=10$ TeV. 
\Cc{{\em [CP: this needs to be changed]}} Assuming blazars follow a similar evolution to quasars, we use $z_\mathrm{max}=5$ \citep{2007ApJ...654..731H}.
...
% \includegraphics[width = .45\textwidth ]{window_gal_qso}
\includegraphics[width = .45\textwidth ]{window_gal_qso}
\caption{Window function for TeV blazar heating from $z=1$ (solid lines), $z=2$ (dashed lines) and $z=4$ (dotted lines) for the galaxy bias model (red) and the quasar bias model (blue). \ALc{ The arrows indicate the minimal and maximal wavenumber modeled in the simulation.}}
\label{fig:window}
\end{figure}
...
\begin{equation}
  \label{eq:flux_recu0}
  J(\mathbf{x})=
  \int_{0}^{2\pi}d\phi\int_{0}^{\pi}\sin\theta d\theta\int_0^{r_{\mathrm{max}}}|\mathbf{x}'-\mathbf{x}|^2 d^3|\mathbf{x}'-\mathbf{x}|
  \frac{\mathcal{E}(\mathbf{x}') }{4\pi |\mathbf{x}'-\mathbf{x}|^2} e^{-\tau}
  =\int_{\Omega} d\Omega\int_0^{r_{\mathrm{max}}} dr' \frac{\mathcal{E}(\mathbf{r}'+\mathbf{x}) }{4\pi } e^{- r'/D_{\mathrm{pp}}},
\end{equation}
where the emissivity $\mathcal{E}$ is an energy per unit time, per unit volume, $\tau=|\mathbf{x}'-\mathbf{x}|/D_{\mathrm{pp}}$ is the optical depth along the line of sight and $D_{\mathrm{pp}}$ is the mean free path for pair production. In the last step, we introduced $\mathbf{r'}=\mathbf{x}'-\mathbf{x}$, $d\Omega=\sin\theta d\theta d\phi$. The differential heating rate is defined as $d\dot{Q}(\mathbf{x})\equiv dJ(\mathbf{x})/D_{\mathrm{pp}}$ such that the volume-integrated heating rate $\dot{Q}$ and its average (in units of erg cm$^{-3}$ s$^{-1}$) are given by 
\begin{equation}
  \label{eq:heating_rate0}
  \dot{Q}(\mathbf{x})=
  \frac{1}{4\pi}   \int_{\Omega}d\Omega\int_0^{r_{\mathrm{max}}} dr' \mathcal{E}(\mathbf{r}'+\mathbf{x})\, \frac{e^{-r'/D_{\mathrm{pp}}}}{D_{\mathrm{pp}}},\quad\mbox{and}\quad
  \bar{\dot{Q}}= \frac{1}{4\pi}   \int_{\Omega}d\Omega\int_0^{r_{\mathrm{max}}} dr' \bar{\mathcal{E}} \,\frac{e^{-r'/D_{\mathrm{pp}}}}{D_{\mathrm{pp}}} ,
\end{equation}
...
\begin{eqnarray}
  ...
  &=&\frac{1}{4\pi\bar{\dot{Q}}D_{\mathrm{pp}}}\int_{\Omega}d\Omega\int_0^{r_{\mathrm{max}}} dr' \delta_E(\mathbf{r}'+\mathbf{x})\bar{\mathcal{E}}\,e^{-r'/D_{\mathrm{pp}}} ,
\end{eqnarray}
where $\delta_E$ are the fluctuations of the TeV photon emissivity. Finally we assume that the fluctuations in the TeV emissivity follow the DM density fluctuations, i.e., $\delta_E=\delta$. This is justified by the tight relation between the mass of the central black hole and the stellar mass of the bulge of the host galaxy\citep{2004ApJ...604L..89H}.
...
\end{eqnarray}
\ALc{Performing a convolution of the density fluctuations with the kernel $C r^{-2} e^{-r/D_{\mathrm{pp}}}$ (with $C$ an arbitrary constant), and using the Fourier convolution theorem, Eq.~\eqref{eq:heat_fluc_newt0} rewrites}
...
\begin{equation}
  \label{eq:heat_fluc_newt2}
  \tilde{\delta}_H(\mathbf{k})=\frac{1}{4\pi\bar{\dot{Q}}D_{\mathrm{pp}}} \int_{\Omega}d\Omega\int_0^{r_{\mathrm{max}}}  dr' \bar{ \mathcal{E}} e^{- r'/D_{\mathrm{pp}}}  \tilde{\delta}_E(\mathbf{k}) e^{-i\mathbf{k}\cdot{\mathbf{r}'}}  ,
\end{equation}
...
\begin{eqnarray}
  ...
  \tilde{\delta}_H(k)&=&  
  \frac{1}{2\bar{\dot{Q}}D_{\mathrm{pp}}} \int_{-1}^{1} d\mu \int_0^{r_{\mathrm{max}}}dr'  \bar{\mathcal{E}} e^{-r'/D_{\mathrm{pp}}} \tilde{\delta}_E(k) e^{-ikr'\mu}
  =\tilde{\delta}_E(k)\frac{\bar{\mathcal{E}}}{D_{\mathrm{pp}}} {\bar{\dot{Q}}}\int_0^{r_{\mathrm{max}}} \frac{\sin(kr')}{kr'} e^{-r'/D_{\mathrm{pp}}}   dr'\\ \nonumber
...
\end{equation}
where $J_E$ is the specific intensity at the origin (in erg s$^{-1}$ cm$^{-2}$ erg$^{-1}$), $E_{\mathrm{min}}$ to $E_{\mathrm{max}}$ is the range over which pair production is possible\Cc{, and $F_E$ denotes intrinsic TeV blazar spectrum of Eq.~\eqref{eq:intrinsic_spectrum}.} The evolution of $J_E$ is dictated by the equation of radiative transfer in an expanding universe
\begin{equation}\label{eq:radiative transfer}
%  \frac{\partial J_E}{\partial t} - E H \frac{\partial J_E}{\partial E} + 3 H J_E + c\kappa J_E/D_{\mathrm{pp}}  = \frac {c}{4\pi} \epsilon_E,
...
\end{equation}
where $H = \dot{a}/a$ is the Hubble parameter, $a$ is the scale factor, and $\epsilon_E$ is the specific proper emissivity.  Quantities indicated by a hat are measured in the comoving frame.
Integrating equation (\ref{eq:radiative transfer}) over all lines of sight, we find 
\begin{equation}\label{eq:expanding J}
  J_E(E, z) = \frac {(1 + z)^3} {4\pi} \int_{\Omega} d\Omega \int_z^{z_{\mathrm{max}}} \frac{d\hat r}{dz'} dz' {\hat{\mathcal{E}}_E(E',z')}e^{-\tau(E,z,z')},
\end{equation}
where $\hat r$ is the comoving distance, $\hat{\mathcal{E}}_E = \epsilon_E/(1+z')^3$ is the comoving specific emissivity defined in Eq.~\eqref{eq:mean_heat}, $E' = E (1+z')/(1+z)$ and $\tau$ is the approximate optical depth given by
\begin{equation}
\label{eq:tau}
\tau(E,z,z')\equiv \tau=\int_z^{z'}dz''\frac{1}{\hat D_{\mathrm{pp}}(E,z'')}\frac{d\hat r}{dz''}.
\end{equation}
An alert reader will recognize that the true optical depth must include the effect of a changing $\hat D_{\rm pp}$ as we integrate from $z$ to $z'$.  Here we make the assumption that $\tau(E',z,z') \propto \hat r$ to derive equation (\ref{eq:expanding J}). This assumption holds because the typical mean free paths for high energy photons are much smaller than the Hubble length.  Hence $E' \approx E$ as the redshift interval over which integrand is nontrivial is small compared to $z$.  
...
\begin{equation}
  ...
  \dot{Q}(z)=\frac{(1+z)^3}{4\pi }\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(E,z)}\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz'\frac{d\hat r}{dz'}\mathcal{\hat E}(E',z') e^{-\tau(E,z,z')}.
\end{equation}
%\sout{
...
\bar{\dot{Q}}=\frac{(1+z)^3}{4\pi}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(E, z)}\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz' \frac{d\hat r}{dz'}\bar{\hat{\mathcal{E}}}(E',z') e^{-\tau(E,z,z')}.
\end{equation}

\begin{eqnarray}
\label{eq:fluc_exp0}
\delta_H(z)&=&\frac{\dot{Q}(z)-\bar{\dot{Q}}}{\bar{\dot{Q}}}=\frac{(1+z)^3c}{4\pi\bar{\dot{Q}}} \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}}\frac{dE}{\hat D_{\mathrm{pp}}(E,z)}\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz'\frac{ \left[ \mathcal{\hat E}(E',z')-\bar{\hat{\mathcal{E}}}(E',z')\right] e^{-\tau(E,z,z')}}{H(z')} \\ \nonumber
&=&\frac{(1+z)^3 c}{4\pi\bar{\dot{Q}}} \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(E,z)}\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz' \frac{\bar{\hat{\mathcal{E}}}(E',z')\delta_E(z') e^{-\tau(E,z,z')}}{H(z')},
\end{eqnarray}
...
At this stage we assume that TeV fluctuations exactly match the dark matter fluctuations $\delta_E=\delta$. We explain in the next section that this is not exactly true and will take into account various corrections. The initial density fluctuations represent a Gaussian random field, which exact properties depend on the earliest stages of the universe prior to recombination \citep{1986ApJ...304...15B,Peebles}. They grow linearly between $z'$ and $z$ following  \citep{ 1977MNRAS.179..351H}
\begin{equation}
  \label{eq:growth}
\delta(z,\hat r)=\delta(z',\hat r)D(z)/D(z'),
\end{equation}
...
\begin{equation}
\label{eq:FT_delta}
\delta_E(z',\hat r)=\delta(z',\hat r)=\delta(z,\hat r)\frac{D(z')}{D(z)}=\delta(z,\hat r)\frac{D(z')}{D(z)}\frac{1}{(2\pi)^3}\int d^3\mathbf{\hat k'} \tilde{\delta}(\mathbf{\hat k'}) e^{-i\mathbf{\hat k'}\cdot\mathbf{\hat r}}.
\end{equation}
And Eq.~\eqref{eq:fluc_exp0} rewrites as
\begin{equation}
\label{eq:heat_fluc_exp0}
\delta_H(z,\hat r)=\frac{(1+z)^3  c}{4\pi\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(E,z)} \int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}}dz' \frac{D(z')}{D(z)} \frac{\bar{\hat{\mathcal{E}}}(E',z')\delta(z',\hat r) e^{-\tau(E,z,z')}}{H(z')}.
\end{equation}
The left hand side yields $\tilde{\delta}(\mathbf{\hat k})$ while the right hand side transforms in a similar fashion to Eq.~\eqref{eq:heat_fluc_newt3}. As the power spectrum of density fluctuations is isotropic, this yields
\begin{equation}
\label{eq:heat_fluc_exp1}
\tilde{\delta}_H(z,\hat k)=\tilde{\delta}(z,\hat k) \frac{(1+z)^3c}{4\pi\bar{\dot{Q}}} \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(E,z)}\int_z^{z_{\mathrm{max}}} dz' \frac{D(z')}{D(z)}\frac{\sin(\hat k\hat r'(z'))}{\hat k \hat r'(z')} \frac{\bar{\hat{\mathcal{E}}}(E',z') e^{-\tau(E,z,z')}} {H(z')}.
\end{equation}
\begin{figure}[h]
\centering
...
% \end{equation}
If galaxies were moving exactly with the Hubble flow, their redshift would yield their exact distance to an observer. However, structures falling towards a central potential of a cluster have an infall velocity towards the central overdensity. This yields the following redsift correction
\begin{equation}
\label{eq:vel_perturb}
dz'=d\hat r\frac{c}{H(z')}\left(1-\frac{d\delta_{v_r}(z')}{d\hat r}\right),
\end{equation}
...
\begin{equation}
\label{eq:mean_heat0}
\dot{Q}(z)=\frac{(1+z)^3c}{4\pi }\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}}\frac{dE}{\hat D_{\mathrm{pp}}(E,z)}\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz'\frac{\mathcal{\bar{\hat E}}(E',z')e^{-\tau(E,z,z')}}{H(z')}\left[1+\left(\frac{D(z')}{D(z)}b(z)\delta(z,\hat r) -\frac{d\delta_{v_r}}{d\hat r}f\right)\right].
\end{equation}
Subtracting the mean heating rate (Eq.~\eqref{eq:mean_exp_heat}) then yields the fluctuations
\begin{equation}
\label{eq:heat_fluc0}
\delta_H(z,\hat r)=\frac{1}{X}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} dE\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}}dz'\frac{dX}{dz'}\frac{D(z')}{D(z)}\left[b(z)\delta(z,\hat r) -\frac{d\delta_{v_r}}{d\hat r}f\right],
\end{equation}
where we introduced
\begin{equation}
\label{eq:def_X}
\frac{dX}{dz'}=\frac{(1+z)^3c}{4\pi \hat D_{\mathrm{pp}}(E,z)}\frac{\mathcal{\bar{\hat E}}(E',z')e^{-\tau(E,z,z')}}{H(z')},
\end{equation}
...
\begin{eqnarray}
\label{eq:heat_fluc0}
\tilde{\delta}_H(z,\mathbf{\hat k})&=&\frac{1}{X}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} dE\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz'\frac{dX}{dz'}\frac{D(z')}{D(z)}\int d^3 \mathbf{\hat k'}\int d^3\mathbf{\hat x}e^{-i\mathbf {\hat k'} \cdot (\mathbf{\hat r'} +\mathbf{ \hat x})} e^{i\mathbf{\hat k \cdot \hat x}} \left[b(z)\delta(z,\mathbf {\hat k'}) -\frac{d\delta_{v_r}}{d\hat r'}f\right]  \\ \nonumber
&=&\frac{1}{X}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} dE\int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz'\frac{dX}{dz'}\frac{D(z')}{D(z)}\left[\int d^3 \mathbf{\hat k'}b(z)\delta(z,\mathbf{\hat  k'})\delta^0(\mathbf{\hat  k}-\mathbf{\hat k'}) e^{-i\mathbf{\hat k'}\cdot\mathbf {\hat r'}} + \int d^3\mathbf{\hat k}'e^{-i\mathbf{\hat k}'\cdot \mathbf{\hat r}'}\delta(z,\mathbf{\hat k}')\delta^0(\mathbf{\hat k}-\mathbf{\hat k}')\mu^2 f \right] \\ \nonumber 
\tilde{\delta}_H(z,\hat k)&=&\frac{2\pi\tilde{\delta}(z,\hat k)}{X}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} dE \int_{\Omega}d\Omega\int_z^{z_{\mathrm{max}}} dz'\frac{dX}{dz'}\frac{D(z')}{D(z)}\int_{-1}^{1}d\mu\left[e^{-i\hat k \hat r'\mu}\delta(z,\hat  k) (b +f \mu^2)\right]\\ \nonumber
%\Biggl(\left(b(z)+\frac{2}{3}\right)+\mu^2\Biggr) e^{-ikr\mu} \\ \nonumber
&=&\frac{4\pi\tilde{\delta}(z,\hat k)}{X}\int_z^{z_{\mathrm{max}}}dz'\frac{dX}{dz'}\frac{D(z')}{D(z)}\left[\left(b(z)+\frac{f}{3}\right)j_0(\hat k \hat r)-\frac{2}{3}f j_2(\hat k \hat r)\right],
\end{eqnarray}
where we have assumed isotropy and homogeneity. We used the spherical Bessel functions
\begin{eqnarray}
\label{eq:bessel}
j_0(x)&=& \frac{\sin(x)}{x}\\
...
\begin{eqnarray}
\label{eq:heat_fluc}
\tilde{W}_H(z,\hat k)&=&\frac{(1+z)^3c }{\bar{\dot{Q}}}\int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{\hat D_{\mathrm{pp}}(E,z)}\int_z^{z_{\mathrm{max}}} dz' \frac{\mathcal{\bar{\hat E}}(E',z')e^{-\tau(E,z,z')}}{H(z')}\frac{D(z')}{D(z)}\left[\left(b(z)+\frac{f}{3}\right)j_0(\hat k \hat r)-\frac{2}{3}fj_2(\hat k \hat r)\right]\\
&=& \frac{\displaystyle \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{{\hat D_{\mathrm{pp}}}(E,z)}\int_z^{z_{\mathrm{max}}} dz'   \frac{\mathcal{\bar{\hat E}}(E',z')e^{-\tau(E,z,z')}}{H(z')}\frac{D(z')}{D(z)}\left[\left(b(z)+\frac{f}{3}\right)j_0(\hat k \hat r)-\frac{2f}{3}j_2(\hat k \hat r)\right]}{\displaystyle \int_{E_{\mathrm{min}}}^{E_{\mathrm{max}}} \frac{dE}{{\hat D_{\mathrm{pp}}}(E,z)}\int_z^{z_{\mathrm{max}}} dz'  \mathcal{\bar{\hat E}}(E',z') \frac{e^{-\tau(E,z,z')}}{H(z')}}.
\end{eqnarray}
\ALc{This equation is similar to \citet{2007MNRAS.376.1680P,2005ApJ...626....1B} but does not contain the unnecessary area correction factor.}
